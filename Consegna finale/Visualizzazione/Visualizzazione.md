...
Innanzitutto abbiamo permesso la visualizzazione di tre reti: 
- la rete che riguarda il topic immigrazione dell'anno 2019;
- la rete che riguarda il topic immigrazione dell'anno 2023; 
- le rete generata dal segregation model con una soglia del 95%;

mettendo di default la rete che abbiamo selezionato per l'analisi, ovvero quella del 2023. La scelta avviene tramite un menù a tendina.
...
- *center*: impostazione della forza attrattiva verso il centro di massa del layout
...
simulation = d3.forceSimulation(currentNodes)
                .force("link", d3.forceLink(currentLinks) // crea una "molla tra i nodi"
                    .id(d => d.id)
                    .distance(2) // distanza tra i nodi
                    .strength(0.3)) // forza del collegamento
                .force("charge", d3.forceManyBody() // forza repulsiva
                    .strength(d => chargeScale(d.degree)).theta(0.9)) 
